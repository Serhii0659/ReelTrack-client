...
function LoginPage() {
    ...
    // State для відображення повідомлень про помилки
    ...
    const handleSubmit = async (e) => {
        ...
        if (!email || !password) {
            setError('Будь ласка, введіть логін та пароль'); // Виводимо повідомлення, якщо поля порожні
            return;
        }

        try {
            await login(email, password); // Викликаємо функцію логіну
            navigate('/home'); // Перенаправляємо користувача на сторінку /home після успішного входу
        } catch (err) {
    ...
    return (
        <div
            className="relative w-screen h-screen bg-neutral-900 text-white overflow-hidden"
